...
class GlobalSysAllocator:
    ...
    def create_blocks(self,gridwise:GridwiseChebyshev):
        ...
        if ncheb == 1:
            bclm0 = self.local_equation.generate_interior_matrices(self.flof, gridwise.cheblist[0],1,1)
            bclm0.trim(left = True,right = True)
            blocks.add_tricolumn(bclm0)
        else:
            bclm0 = self.local_equation.generate_interior_matrices(self.flof, gridwise.cheblist[0],1,ps[1])
